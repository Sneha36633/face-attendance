...
def load_images():
    ...
    # os.walk automatically sub-folders ke andar jaata hai
        ...
        for file in files:
            if file.endswith(('.jpg', '.jpeg', '.png')):
                ...
                if img is not None:
                    ...
                    sections.append(os.path.basename(root)) # Folder ka naam section hai
...
def register():
    # request.form.get use karne se KeyError nahi aayega
    name = request.form.get('name')
    section = request.form.get('section')

    if not name or not section:
        return "Error: Please send both 'name' and 'section' keys in the form-data!", 400
    
    name = name.strip()
    section = section.strip().upper()

    # Section folder create karein
    ...
    while True:
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            ...
            if len(face_locations) == 0:
                msg = "Error: Face detection failed."
                break
